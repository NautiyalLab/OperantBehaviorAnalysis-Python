...
column_list = ['Subject', 'Day', 'Dippers', 'Dippers Retrieved', 'Retrieval Latency', 'Small Rewards', 'Large Rewards',
...
def crf_function(loaded_file, i):
    ...
    num_small_rewards = eventcode.count('SmallReward')
    num_large_rewards = eventcode.count('LargeReward')
...
#        (lever_press_rate, iti_rate) = cue_iti_responding(timecode, eventcode, 'StartSession', 'EndSession', 'RPressOn')
    ...
    df2 = pd.DataFrame([[loaded_file['Subject'], int(i + 1), float(dippers),
                         float(dippers_retrieved), float(retrieval_latency), num_small_rewards, num_large_rewards, float(left_presses),
