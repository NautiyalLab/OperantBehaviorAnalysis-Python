from operantanalysis import loop_over_days, extract_info_from_file, reward_retrieval, cue_responding_duration, \
    total_head_pokes, duration_across_trace_iti
import pandas as pd
...
def Trace_dur_function(loaded_file, i):
    """
    ...
    (ind_dur, tot_dur, ind_dur_iti, tot_dur_iti) = cue_responding_duration(timecode, eventcode, 'EndTrial',
                                                                           'DipOn', 'PokeOn1', 'PokeOff1')
    total_pokes = total_head_pokes(eventcode)
    (all_cue_length_poke_dur, all_iti_length_poke_dur, subtracted_poke_dur) = \
        duration_across_trace_iti(timecode, eventcode, 'EndTrial', 'DipOn', 'PokeOn1', 'PokeOff1', 8)
...
(days, df) = loop_over_days(column_list, Trace_dur_function)
print(df.to_string())
