from operantanalysis import loop_over_days, extract_info_from_file, cue_iti_responding_subset
import pandas as pd


column_list = ['Subject', 'Condition', 'Day', 'CS+ Pokes Rate', 'CS+ ITI', 'CS- Pokes Rate',
               'CS- ITI', 'CS+ Elevation Score', 'CS- Elevation Score']


def PIT_training_function(loaded_file, i):
    """
    :param loaded_file: file output from operant box
    :param i: number of days analyzing
    :return: data frame of all analysis extracted from file (one animal)
    """

    (timecode, eventcode) = extract_info_from_file(loaded_file, 500)
    (A_responding, A_iti) = cue_iti_responding_subset(timecode, eventcode, 'ExcitorATrialStart', 'ExcitorATrialEnd', 'PokeOn1', 15)
    (B_responding, B_iti) = cue_iti_responding_subset(timecode, eventcode, 'ExcitorBTrialStart', 'ExcitorBTrialEnd', 'PokeOn1', 15)
    df2 = pd.DataFrame([[loaded_file['Subject'], loaded_file['MSN'], int(i + 1),
                         float(A_responding), float(A_iti), float(B_responding), float(B_iti),
                         float(A_responding-A_iti), float(B_responding-B_iti)]], columns=column_list)

    return df2


(days, df) = loop_over_days(column_list, PIT_training_function)
print(df.to_string())
df.to_excel("output.xlsx")